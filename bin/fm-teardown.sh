...
if [ "$KIND" = secondmate ]; then
  ...
  pending_replies_recovery_validate recheck ||
    {
      echo "error: local pending-reply recovery paths changed; preserving the secondmate home and route" >&2
      exit 1
    }
  ...
  if [ "$PENDING_REPLIES_DIR_PRESENT" -eq 1 ]; then
    pending_replies_cleanup_for_task "$STATE/pending-replies" "$PENDING_REPLIES_DIR_REAL" ||
      {
        echo "error: local pending-reply cleanup failed; preserving the secondmate route for retry" >&2
        exit 1
      }
  fi
